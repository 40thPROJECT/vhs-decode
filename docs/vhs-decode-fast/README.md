...
### Dropped when rebasing onto current upstream

Three changes that were in this fork are gone, because upstream arrived at the
...
Both divisions cancel: the expression is `line_length / outlinelen`.  Upstream
reached the same conclusion independently and rewrote the function - their
current code is `scale = line_length * inv_outlinelen`, with no division left to
fail - so this fork carries no fix of its own for it.  Rebasing onto their work
is what removes the crash.
