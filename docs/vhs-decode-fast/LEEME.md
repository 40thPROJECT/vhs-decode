...
### Comprobación de niveles en una pasada — `vhsdecode/addons/resync.py`

`check_levels` hacía dos `np.argwhere` sobre el `demod_05` del campo entero,
reservando dos arrays de índices, varias veces por campo. Ahora cuenta ambos
umbrales en un solo bucle sin reservar memoria.

### `scipy.fft` en vez de `numpy.fft` — `vhsdecode/process.py`, `nonlinear_filter.py`

Un cambio de una línea. **Medido: ~23 % menos tiempo en las FFT.**
...
### Fallback en Python para el módulo Rust — `vhsdecode/rust_fallback.py`

`vhsdecode.demod` y `vhsdecode.main` importaban `vhsd_rust` sin condiciones, así
que un checkout del código fuente sin toolchain de cargo no arrancaba siquiera.
El fallback aporta equivalentes en numpy/numba. Se sigue prefiriendo `vhsd_rust`
cuando está: vale un +70 % él solo.
