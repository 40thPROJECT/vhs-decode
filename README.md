...
Rebased on upstream commit `2f21e8ed` (25 July 2026). Everything upstream still
works the same way — the original documentation is kept as
...
Measured against upstream as it stood on 16 July 2026. They have moved on since;
the changes still apply cleanly and the code is unchanged, but the percentages
have not been re-measured on the newer base.
...
* The crash itself was a divide-by-zero in upstream's NTSC burst sync
  (`_sync_to_burst`) on a degenerate field. Upstream has since rewritten that
  function and the division is gone, so this fork carries no fix of its own for
  it — only the detection above and the repair tool below.
...
Rebasado sobre el commit `2f21e8ed` del original (25 de julio de 2026). Todo lo
del proyecto original sigue funcionando igual — su documentación se conserva en
[README_vhs-decode.md](README_vhs-decode.md).
...
Medido contra el original tal como estaba el 16 de julio de 2026. Desde entonces
ha avanzado; los cambios siguen aplicando limpios y el código es el mismo, pero
los porcentajes no se han vuelto a medir sobre la base nueva.
...
* El crash en sí era una división por cero en el sincronismo de burst NTSC del
  original (`_sync_to_burst`) ante un campo degenerado. El proyecto original ya
  reescribió esa función y la división desapareció, así que este fork no lleva
  arreglo propio para eso — solo la detección de arriba.
